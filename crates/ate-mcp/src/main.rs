...
//!
//! This binary is Windows-only: it spawns `wsl.exe` to talk to the daemon.
//!
...
#[cfg(not(windows))]
fn main() {
    eprintln!("ate-mcp is only supported on Windows");
    std::process::exit(1);
}
...
mod implementation {
    ...
    pub fn run() {
        ...
        if let Err(error) = runtime.block_on(main_loop()) {
    ...
    fn to_text(content: &Value) -> String {
        ...
        serde_json::to_string_pretty(content)
            .unwrap_or_else(|_| "failed to serialize tool output".to_string())
    }
    ...
    async fn main_loop() -> Result<(), Box<dyn std::error::Error>> {
        let distribution =
            std::env::var("ATE_MCP_DISTRIBUTION").unwrap_or_else(|_| "Ubuntu".into());
